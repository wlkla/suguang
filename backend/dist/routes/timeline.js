...
// 获取记忆的时间线分析
router.get('/analysis/:memoryId', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        // 获取该记忆的所有时间线分析
        const timelineAnalyses = await prisma_1.prisma.timelineAnalysis.findMany({
            where: {
        ...
        // 计算实际的对话次数（排除初始记录）
        const actualConversations = timelineAnalyses.filter(analysis => analysis.analysisStage !== 'initial').length;
        res.json({
            memoryTitle: memoryRecord.title,
            timeline,
...
router.post('/generate', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const existingAnalyses = await prisma_1.prisma.timelineAnalysis.findMany({
            where: {
                memoryRecordId,
                userId
            },
        ...
        if (existingAnalyses.length === 0) {
            ...
            // 过滤掉初始分析，计算实际对话次数
            const conversationAnalyses = existingAnalyses.filter(analysis => analysis.analysisStage !== 'initial');
        ...
        // 检查是否已存在相同阶段的分析，避免重复
        const existingStageAnalysis = existingAnalyses.find(analysis => analysis.analysisStage === actualStage);
        ...
        // 使用AI生成心理分析
        const analysis = await aiService_1.aiService.analyzePsychology(memoryRecord, analysisConversationData, actualStage);
        // 保存分析结果
        const timelineAnalysis = await prisma_1.prisma.timelineAnalysis.create({
            data: {
                userId,
                memoryRecordId,
...
// 清理记忆的重复分析记录
router.delete('/cleanup/:memoryId', auth_1.authenticateToken, async (req, res) => {
    try {
        const memoryId = parseInt(req.params.memoryId);
        const userId = req.user.id;
        // 验证记忆记录是否存在且属于当前用户
        ...
        // 获取该记忆的所有分析，按创建时间排序
        const analyses = await prisma_1.prisma.timelineAnalysis.findMany({
            where: {
                memoryRecordId: memoryId,
                userId
            },
        ...
        if (toDelete.length > 0) {
            await prisma_1.prisma.timelineAnalysis.deleteMany({
                where: {
                    id: { in: toDelete }
                }
            });
