...
router.post('/generate', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // 保存分析结果
    const savedAnalysis = await prisma.thoughtAnalysis.create({
      data: {
        userId,
        analysisType,
        timeRange,
...
router.get('/', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const analyses = await prisma.thoughtAnalysis.findMany({
      where: { userId },
    ...
    const total = await prisma.thoughtAnalysis.count({
      where: { userId }
    })
...
router.get('/:id', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const analysis = await prisma.thoughtAnalysis.findFirst({
      where: {
        id: analysisId,
        userId
      }
...
router.delete('/:id', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const existingAnalysis = await prisma.thoughtAnalysis.findFirst({
      where: {
        id: analysisId,
        userId
      }
    ...
    await prisma.thoughtAnalysis.delete({
      where: { id: analysisId }
    })
...
// 模拟分析生成函数 (实际应该集成AI分析)
async function generateAnalysis(memoryRecords: any[], analysisType: string, timeRange: string) {
  // 计算情绪趋势
...
function calculateEmotionalTrend(records: any[]) {
...
function getEmotionalDescription(avgMood: number): string {
  if (avgMood < 2.5) return '情绪波动较大，多焦虑'
  if (avgMood < 3.5) return '开始有所改善，更加稳定'
  if (avgMood < 4) return '明显提升，积极情绪增多'
  return '整体乐观，内心更加平和'
}

function identifyKeyMoments(records: any[]) {
...
function generateInsights(records: any[]) {
  return {
    growth: [
      '对工作的态度从焦虑转向更加平和和理性',
      '在人际关系中学会了更好的边界设定',
...
function analyzeInfluences(records: any[]) {
  return {
    internal: [
      '自我反思能力的提升',
      '对心理健康的重视',
